...
Today, this class of query is either answered inconsistently by support teams, left for users to dig out of dense factsheets and SID documents themselves, or picked up from third-party sources that can be outdated or inaccurate. A facts-only assistant grounded in official sources reduces this friction while keeping the product firmly on the right side of advisory compliance.
...
- Select one Asset Management Company (AMC): **SBI Mutual Fund**
- Select 3–4 schemes under SBI Mutual Fund, spanning categories:
...
1. **Working Prototype** — App/notebook link, or a ≤3-minute demo video if hosting isn't possible
2. **Source List** — CSV/MD file listing the 15–25 URLs used
3. **README Document** — Setup instructions, selected AMC and schemes, architecture overview (RAG approach), known limitations
